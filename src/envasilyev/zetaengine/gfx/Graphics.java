...
public class Graphics {
	...
	public void drawBitmap(Bitmap bitmap, Transform transform) {
		...
		if (transform.getRotation() == 0) {
			drawBitmap(x_offset, y_offset, bitmap);
			return;
		}
		else
		if (transform.getRotation() == -90 || transform.getRotation() == 270) {
			...
			drawBitmap(x_offset, y_offset, result);
			return;
		}
		else
		if (transform.getRotation() == 90 || transform.getRotation() == -270) {
	...
	public void drawString(int x, int y, int color, String text) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			if (c == '\n') {
				...
				BitmapFont.Glyph fontChar = font.getChar((int)text.charAt(i));

				int xOffset = 1;
				if (fontChar.xOffset > 0) {
					xOffset = fontChar.xOffset;
				}
				int xRender = xCursor + xOffset;

				int yOffset = 1;
				if (fontChar.yOffset > 0) {
					yOffset = fontChar.yOffset;
				}

				int yRender = yCursor + yOffset;
